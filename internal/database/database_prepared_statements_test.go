...
func TestDataStorage_getCreatePreparedstatements(t *testing.T) {
	type fields struct {
		db         *sql.DB
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := ds.getCreatePreparedstatements(); (err != nil) != tt.wantErr {
				t.Errorf("DataStorage.getCreatePreparedstatements() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
